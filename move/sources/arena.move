...
public fun battle(hero: Hero, arena: Arena, ctx: &mut TxContext) {
    ...
    let (winner, winner_hero_id, loser_hero_id) = if (hero.hero_power() > warrior.hero_power()) {
        (ctx.sender(), object::id(&hero), object::id(&warrior))
    } else {
        (owner, object::id(&warrior), object::id(&hero))
    };
    ...
    event::emit(ArenaCompleted {
        winner_hero_id: winner_hero_id,
        loser_hero_id: loser_hero_id,
        timestamp: ctx.epoch_timestamp_ms(),
    });
